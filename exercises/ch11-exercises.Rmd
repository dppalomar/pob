...
\begin{exercise}
Newton's method requires computing the direction $\bm{d} = {\sf H}^{-1}\nabla f$ or, equivalently, solving the system of linear equations ${\sf H}\bm{d} = \nabla f$ for $\bm{d}$. Explore whether a more efficient solution is possible exploiting the structure of the gradient and Hessian:
$$
\begin{aligned}
\nabla f &= \bSigma\bm{x} - \bm{b}/\bm{x}\\
{\sf H}  &= \bSigma + \textm{Diag}(\bm{b}/\bm{x}^2).
\end{aligned}
$$
\end{exercise}
